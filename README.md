...
**41 tools** are registered across 9 functional areas covering the full daily workflow of a SOC team.
...
Fully quit Claude Desktop (tray icon → **Quit**, not just close the window), then relaunch. Open a new chat — you should see a tools icon at the bottom of the input box. Clicking it shows `wazuh` with all 41 tools listed.
...
**Keep `WAZUH_ALLOW_WRITES=false` in production.** `restart_agent` and `run_active_response` are disabled by default. Enable only when explicitly delegating remediation to the AI.
